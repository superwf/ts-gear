...
exports.parseGenericType = exports.checkAndUpdateRequestRef = exports.checkAndUpdateDefinitionTypeName = void 0;
...
exports.checkAndUpdateDefinitionTypeName = (projectGlobal) => {
        ...
        if (genericType_1.hasGenericSymbol(definitionName)) {
            if (definition.schema) {
                ...
                if (allTypeParametersInSchemaRef) {
                     ...
                     * */
                    definitionMap[genericType_1.removeGenericSymbol(definitionName)] = {
                        typeName: parentNode.name,
                        schema: definition.schema,
                        typeParameters: definition.typeParameters,
                    };
                    // console.log(parentNode.name, definitionName)
                    ...
                    typeParameters.forEach((typeName) => {
                        if (!(typeName in definitionMap)) {
                            ...
                            nodes.forEach((node) => {
                                patchGlobalDefinitionMap_1.patchGlobalDefinitionMap({ typeName: node.name, definitionMap });
                            });
                        }
                    });
                }
                        ...
                        if (!nodes.every((node) => node.name in definitionMap)) {
                            ...
                            name
                                ...
                                if (!(n in definitionMap)) {
                                    patchGlobalDefinitionMap_1.patchGlobalDefinitionMap({ typeName: n, definitionMap });
                                }
                            });
                        }
                    }
                    else {
                        patchGlobalDefinitionMap_1.patchGlobalDefinitionMap({ typeName: value, definitionMap });
                    }
                }
            });
        });
    ...
    traverseSchema_1.traverseSchema(definitionMap, ({ value, key, parent }) => {
        if (key === '$ref' && genericType_1.hasGenericSymbol(value) && value in definitionMap) {
