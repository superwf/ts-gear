...
exports.sampleFromSchema = (schema, definitionMap) => {
    ...
    if (!type) {
        if (properties) {
            ...
            if (definitionSchema) {
                return exports.sampleFromSchema(definitionSchema, definitionMap);
            }
            return '';
        }
        else if (schemaSchema) {
            return exports.sampleFromSchema(schemaSchema, definitionMap);
        }
        else {
            return '';
        }
    ...
    if (type === 'object') {
        ...
        Object.getOwnPropertyNames(props).forEach(name => {
            if (!(props[name] && props[name].deprecated)) {
                obj[name] = exports.sampleFromSchema(props[name], definitionMap);
            }
        });
        if (additionalProperties === true) {
            ...
            const additionalPropVal = exports.sampleFromSchema(additionalProps, definitionMap);
            for (let i = 1; i < 4; i += 1) {
    ...
    if (type === 'array') {
        if (Array.isArray(items.anyOf)) {
            return items.anyOf.map((i) => exports.sampleFromSchema(i, definitionMap));
        }
        if (Array.isArray(items.oneOf)) {
            return items.oneOf.map((i) => exports.sampleFromSchema(i, definitionMap));
        }
        return [exports.sampleFromSchema(items, definitionMap)];
    }
    if (schema.enum) {
        if (schema.default) {
