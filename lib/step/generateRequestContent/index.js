...
exports.generateRequestContent = (spec, project) => {
    ...
    const { requestMap, definitionMap } = projectGlobalVariable_1.getGlobal(project);
    const resultContent = [];
    Object.getOwnPropertyNames(requestMap).forEach(requestFunctionName => {
        ...
        const mockStatment = `return Promise.resolve(${JSON.stringify(generateMockData_1.generateMockData(request, definitionMap))} as any)`;
