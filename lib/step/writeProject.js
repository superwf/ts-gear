...
exports.writeProject = (project, tsGearConfigPath) => {
    ...
    const enumContent = Object.values(enumMap)
        ...
        .join(os_1.EOL);
    prettierWrite_1.prettierWrite([warningComment_1.warningComment, enumContent, definitionContent].join(os_1.EOL), path_1.join(dest, constant_1.targetFileNames.definition), project.prettierConfig);
    const requestContent = Object.getOwnPropertyNames(requestMap)
        .map(name => {
        return requestMap[name].typescriptContent;
    })
        .join(os_1.EOL);
    const requesterResult = requester_1.requester(project, tsGearConfigPath);
    prettierWrite_1.prettierWrite([
        ...
        requestContent,
    ].join(os_1.EOL), path_1.join(dest, constant_1.targetFileNames.request), project.prettierConfig);
    prettierWrite_1.prettierWrite([warningComment_1.warningComment, projectIndex_1.projectIndex].join(os_1.EOL), path_1.join(dest, constant_1.targetFileNames.index), project.prettierConfig);
};
